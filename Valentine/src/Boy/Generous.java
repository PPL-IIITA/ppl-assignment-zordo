...
public class Generous extends Boy{
    ...
    public Generous(String name, int budget, int intelligence, int attractiveness, String type, int minattract) {
        super(name, budget, intelligence, attractiveness, type, minattract);
    }

    @Override
    public void setHappiness() {
        double happiness = getGf().getHappiness();
            ...
            if(sum<getBudget()){
                ...
                if(sum>getBudget()&&!(girl.getGiveme().isEmpty())){
                    sum-=gift[j].getPrice();
                        ...
                        String kya1 =getName()+" gifted object #"+gift[j]+"@"+df.format(dateobj)+" to "+girl.getName()+" "+'\r'+'\n';
    ...
    public void gifther2(Girl girl, int gif, Gift[] gift) {
        ...
        int i,j,flag1 = -1,flag2 = -1,flag3 = -1;
        int sum = 0;
        File out = new File("Gifting.txt");
        DateFormat df = new SimpleDateFormat("dd/MM/yy HH:mm:ss");
        Date dateobj = new Date();
