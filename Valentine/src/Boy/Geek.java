...
public class Geek extends Boy{
    ...
    public Geek(String name, int budget, int intelligence, int attractiveness, String type, int minattract) {
        super(name, budget, intelligence, attractiveness, type, minattract);
    }

    @Override
    public void setHappiness() {
        double happiness = getGf().getIntelligence();
     ...
     public void gifther(Girl girl,int gif,Gift[] gift){
        ...
        for(j=0;j<gif;j++){
            if(sum<girl.getMaintaince()){
                ...
                String kyay =getName()+" gifted object #"+gift[j]+"@"+df.format(dateobj)+" to "+girl.getName()+" "+'\r'+'\n';
                ...
                for(;j<gif;j++){
                    ...
                    {   sum+=gift[j].getPrice();
                        if(sum<=getBudget()){
                            girl.getGiveme().add(gift[j]);
                            String kyax =getName()+" gifted object #"+gift[j]+"@"+df.format(dateobj)+" to "+girl.getName()+" "+'\r'+'\n';
